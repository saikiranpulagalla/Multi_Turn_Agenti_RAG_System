...
Provides intent detection for global queries and uses a RAPTOR-style selection for representative chunks.
"""
...
class RAGAgent:
    def __init__(self, vectorstore=None, memory_service=None):
        ...
        # Initialize tracer (top-level import ensures get_tracer is defined)
    ...
    def ingest_pdf(self, pdf_path: str, doc_id: str = None):
        """Ingest a PDF, create chunks and embeddings, and upsert to the vectorstore."""
    ...
    def retrieve(self, query: str, top_k_parents: Optional[int] = None, top_k_chunks: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if is_intent_query:
                # Intent-based: return all chunks for LLM to synthesize global answer
                logger.info(f"Intent-based query detected: '{query[:60]}' - returning all available chunks")
                ...
                # Return all chunks from top parent docs (no selection/filtering for intent queries)
                all_chunks = assemble_late_chunks(parent_ids, self.chunk_index, max_chunks=top_k_chunks)
                if not all_chunks:
                    return []
                ...
                logger.info(f"Retrieved {len(all_chunks)} chunks for intent query (no filtering): {query[:80]}")
                return all_chunks
            ...
            RELEVANCE_THRESHOLD = 0.15  # tuned for all-MiniLM-L6-v2
            ...
            if chunks:
                try:
                    ...
                    if not filtered_chunks:
                        logger.info(f"No chunks above relevance threshold ({RELEVANCE_THRESHOLD}) for topic query: {query[:80]}")
